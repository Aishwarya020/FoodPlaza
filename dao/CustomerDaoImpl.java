...
public class CustomerDaoImpl implements CustomerDao {
	...
	public boolean updateCustomer(Customer customer) {
      try {
			...
			PreparedStatement ps=con.prepareStatement("Update Customer20829 set custname=?, cusemailid=?, custpasswor=?, custaddress=?, custnumber=? where custid=?");
	        ps.setString(1, customer.getCustname());
	        ps.setString(2, customer.getCustemailid());
	        ps.setString(3, customer.getPassword());
	        ps.setString(4, customer.getAddress());
  ...
  try {
	...
	public Customer searchCustomerById(int custid) {
      try {
			...
			if (rs.next())
			{
				Customer customer=new Customer(rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6));
	...
	public Customer searchCustomerByEmailId(String custemailid) {
     try {
			...
			PreparedStatement ps=con.prepareStatement("Select * from Customer20829 where custemailid=?");
